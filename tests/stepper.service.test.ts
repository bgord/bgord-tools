...
import { Stepper } from "../src/stepper.service";

describe("Stepper", () => {
  describe("validations", () => {
    test("invalid max", () =>
      expect(() => new Stepper({ total: 1.5 })).toThrow("Total value is not an integer"));

    test("negative max", () =>
      expect(() => new Stepper({ total: -1 })).toThrow("Total value should be greater than one"));

    test("max equal zero", () =>
      expect(() => new Stepper({ total: 0 })).toThrow("Total value should be greater than one"));
