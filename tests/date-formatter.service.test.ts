...
describe("DateFormatters", () => {
  test("datetime formats date with date and time", () => {
    expect(DateFormatters.datetime(testDate)).toBe("2024/06/01 15:30");
  });

  test("date formats date with just year/month/day", () => {
    expect(DateFormatters.date(testDate)).toBe("2024/06/01");
  });

  test("monthDay formats date with just month/day", () => {
    expect(DateFormatters.monthDay(testDate)).toBe("06/01");
  });
  ...
  describe("relative", () => {
    ...
    test("returns relative time with suffix", () => {
      expect(DateFormatters.relative(testDate)).toBe("5 minutes ago");
    });
  });
});
