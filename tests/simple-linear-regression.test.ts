...
describe("SimpleLinearRegression", () => {
  ...
  describe("validations", () => {
    ...
    test("At least two pairs needed - empty", async () => {
      expect(() => SimpleLinearRegression.fromPairs([])).toThrow("At least two pairs needed");
    });

    test("At least two pairs needed - one pair", async () => {
      expect(() => SimpleLinearRegression.fromPairs([{ x: 1, y: 2 }])).toThrow("At least two pairs needed");
    });
  });
});
