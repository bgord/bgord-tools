...
describe("DirectoryPathAbsoluteSchema", () => {
  ...
  test("rejects non-string - null", () => {
    expect(() => v.parse(DirectoryPathAbsoluteSchema, null)).toThrow("directory.path.absolute.type");
  });

  test("rejects non-string - number", () => {
    expect(() => v.parse(DirectoryPathAbsoluteSchema, 42)).toThrow("directory.path.absolute.type");
  });

  test("rejects too long", () => {
    expect(() => v.parse(DirectoryPathAbsoluteSchema, `/${"a".repeat(512)}`)).toThrow(
      "directory.path.absolute.too.long",
    );
  });

  test("rejects empty", () => {
    expect(() => v.parse(DirectoryPathAbsoluteSchema, "")).toThrow("directory.path.absolute.empty");
  });

  test("rejects empty segment", () => {
    expect(() => v.parse(DirectoryPathAbsoluteSchema, "/tmp//app///users")).toThrow(
      "directory.path.absolute.bad.segments",
    );
  });

  test("rejects trailing slash", () => {
    expect(() => v.parse(DirectoryPathAbsoluteSchema, "/tmp/app/")).toThrow(
      "directory.path.absolute.trailing.slash",
    );
  });

  test("rejects relative path", () => {
    expect(() => v.parse(DirectoryPathAbsoluteSchema, "tmp/app")).toThrow(
      "directory.path.absolute.leading.slash",
    );
  });
